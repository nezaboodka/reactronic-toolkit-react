      ...
      <div className={css.main}>
        <AreaRect hint={'All'} area={scroll.allCells} px={scroll.all} key={`all-${counter}`}
          ...
          <AreaRect hint={'Surface'} area={scroll.surfaceCells} px={scroll.surface} inner={scroll.loaded} key={`surface-${counter}`}
            ...
            <AreaRect hint={'Loaded'} area={scroll.loadedCells} px={scroll.loaded} key={`buffer-${counter}`}
              ...
              <AreaRect hint={'Viewport'} area={scroll.windowCells} px={scroll.window} key={`viewport-${counter}`}
        ...
        <AreaRect hint={'Surface'} area={scroll.surfaceCells} px={scroll.surface} key={`canvas-${counter}`}
          ...
          <div>
            ...
            <div>
              Thumb Pixel = {num(scroll.thumb.scaleBy(scroll.surfaceToWindowFactor).y, 3)}
            </div>
            <div>
              Viewport-to-Canvas = {num(scroll.window.scaleBy(scroll.allToSurfaceFactor).y, 15)}
            </div>
            ...
            <div>{num(scroll.windowToSurfaceFactor.y, 1)} <i>surface pixels in a single viewport pixel out of</i> {num(scroll.window.size.y)}</div>
            <div>{num(scroll.windowToAllFactor.y, 1)} <i>all data pixels in a single viewport pixel out of</i> {num(scroll.window.size.y)}</div>
            <div>{num(scroll.windowToAllFactor.y / scroll.resolution.y, -3)} <i>cells in a single viewport pixel out of</i> {num(scroll.window.size.y)}</div>
            <br/>
          </div>
        </AreaRect>
      </div>
