      ...
      <div className={css.main}>
        <AreaRect hint={'All Data'} area={t.allCells} px={t.all} key={`all-${counter}`}
          ...
          <AreaRect hint={'Surface'} area={t.surfaceCells} px={t.surface} inner={t.loaded} key={`surface-${counter}`}
            ...
            <AreaRect hint={'Buffer'} area={t.bufferCells} px={t.buffer} key={`buffer-${counter}`}
              ...
              <AreaRect hint={'Viewport'} area={t.viewCells} px={t.view} key={`viewport-${counter}`}
        ...
        <AreaRect hint={'Surface'} area={t.surfaceCells} px={t.surface} key={`canvas-${counter}`}
          ...
          <div>
            ...
            <div>
              Thumb Pixel = {num(t.thumb.scaleBy(t.surfaceToViewFactor).y, 3)}
            </div>
            <div>
              Viewport-to-Canvas = {num(t.view.scaleBy(t.allToSurfaceFactor).y, 15)}
            </div>
            ...
            <div>{num(t.viewToSurfaceFactor.y, 1)} <i>canvas pixels in a single viewport pixel out of</i> {num(t.view.size.y)}</div>
            <div>{num(t.viewToAllFactor.y, 1)} <i>all data pixels in a single viewport pixel out of</i> {num(t.view.size.y)}</div>
            <div>{num(t.viewToAllFactor.y / t.resolution.y, -3)} <i>cells in a single viewport pixel out of</i> {num(t.view.size.y)}</div>
            <br/>
          </div>
        </AreaRect>
      </div>
